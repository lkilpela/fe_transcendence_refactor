  ...
  try {
    ...
    if (!user) {
      ...
      const usernameTaken = db
        ...
        .get(googleUser.given_name);

      if (usernameTaken) {
        // If taken, fallback to using email as username
        db.prepare(`INSERT INTO users (username, email) VALUES (?, ?)`).run(
          googleUser.email,
          googleUser.email
        );
      } else {
    ...
    if (!user) {
      return reply
        .code(401)
        .send({ error: "Google sign-in failed to create user" });
    }
    ...
    if (error.response?.data?.error) {
      return reply.code(400).send({
        message: `Google OAuth error: ${error.response.data.error}`,
      });
    }
